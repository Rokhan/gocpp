...
namespace golang::runtime
{
    ...
    struct profBuf* newProfBuf(int hdrsize, int bufwords, int tags)
    {
        ...
        {
            go_throw("newProfBuf: buffer too large"_s);
        }
        int i = {};
    ...
    void rec::close(golang::runtime::profBuf* b)
    {
        ...
        {
            go_throw("runtime: profBuf already closed"_s);
        }
    ...
    std::tuple<gocpp::slice<uint64_t>, gocpp::slice<unsafe::Pointer>, bool> rec::read(golang::runtime::profBuf* b, golang::runtime::profBufReadMode mode)
    {
        ...
        if(ntag == 0)
        {
            go_throw("runtime: malformed profBuf buffer - tag and data out of sync"_s);
        }
        ...
        for(; di < len(data) && data[di] != 0 && ti < len(tags); )
        {
            ...
            {
                go_throw("runtime: malformed profBuf buffer - invalid size"_s);
            }
            di += int(data[di]);
            ti++;
        }
