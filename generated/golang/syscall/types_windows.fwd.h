...
namespace golang::syscall
{
    ...
    const Signal SIGILL = Signal(0x4);
    const Signal SIGTRAP = Signal(0x5);
    const Signal SIGABRT = Signal(0x6);
    const Signal SIGBUS = Signal(0x7);
    const Signal SIGFPE = Signal(0x8);
    const Signal SIGKILL = Signal(0x9);
    const Signal SIGSEGV = Signal(0xb);
    const Signal SIGPIPE = Signal(0xd);
    const Signal SIGALRM = Signal(0xe);
    const Signal SIGTERM = Signal(0xf);
    struct Overlapped;
    struct StartupInfo;
