    ...
    void queuefinalizer(unsafe::Pointer p, funcval* fn, uintptr_t nret, _type* fint, ptrtype* ot)
    {
        ...
        if(finq == nullptr || finq->cnt == uint32_t(len(finq->fin)))
        {
            if(finc == nullptr)
            {
                ...
                if(finptrmask[0] == 0)
                {
                    if((unsafe::Sizeof(finalizer {}) != 5 * goarch::PtrSize || unsafe::Offsetof(finalizer {}.fn) != 0 || unsafe::Offsetof(finalizer {}.arg) != goarch::PtrSize || unsafe::Offsetof(finalizer {}.nret) != 2 * goarch::PtrSize || unsafe::Offsetof(finalizer {}.fint) != 3 * goarch::PtrSize || unsafe::Offsetof(finalizer {}.ot) != 4 * goarch::PtrSize))
                    {
        ...
        for(; ; )
        {
            ...
            for(; fb != nullptr; )
            {
                for(auto i = fb->cnt; i > 0; i--)
                {
                    auto f = & fb->fin[i - 1];
                    abi::RegArgs regs = {};
                    auto framesz = unsafe::Sizeof((go_any)(nullptr)) + f->nret;
                    if(framecap < framesz)
                    {
