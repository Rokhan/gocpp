...
namespace golang::strconv
{
    ...
    void ryuFtoaFixed32(struct decimalSlice* d, uint32_t mant, int exp, int prec)
    {
        if(prec < 0)
        {
            gocpp::panic("ryuFtoaFixed32 called with negative prec"_s);
        }
        if(prec > 9)
        {
    ...
    gocpp::array<uint64_t, 20> uint64pow10 = gocpp::array<uint64_t, 20> {
    ...
    void ryuFtoaShortest(struct decimalSlice* d, uint64_t mant, int exp, struct floatInfo* flt)
    {
        ...
        if(flt == & float32info)
        {
            ...
            uint32_t dc32 = {};
            uint32_t du32 = {};
            std::tie(dl32, std::ignore, dl0) = mult64bitPow10(uint32_t(ml), e2, q);
            std::tie(dc32, std::ignore, dc0) = mult64bitPow10(uint32_t(mc), e2, q);
            std::tie(du32, e2, du0) = mult64bitPow10(uint32_t(mu), e2, q);
            std::tie(dl, dc, du) = std::tuple{uint64_t(dl32), uint64_t(dc32), uint64_t(du32)};
        }
        else
        {
            std::tie(dl, std::ignore, dl0) = mult128bitPow10(ml, e2, q);
            std::tie(dc, std::ignore, dc0) = mult128bitPow10(mc, e2, q);
