...
namespace golang::runtime
{
    ...
    // in the pollInfo.
    bool rec::closing(pollInfo i)
    {
        return i & pollClosing != 0;
    }

    bool rec::eventErr(pollInfo i)
    {
        return i & pollEventErr != 0;
    }

    bool rec::expiredReadDeadline(pollInfo i)
    {
        return i & pollExpiredReadDeadline != 0;
    }

    bool rec::expiredWriteDeadline(pollInfo i)
    {
    ...
    void rec::publishInfo(pollDesc* pd)
    {
        uint32_t info = {};
        if(pd->closing)
        {
    ...
    // (issue #59545).
    void rec::setEventErr(pollDesc* pd, bool b, uintptr_t seq)
    {
    ...
    std::tuple<pollDesc*, int> poll_runtime_pollOpen(uintptr_t fd)
    {
        ...
        {
            go_throw("runtime: blocked write on free polldesc"_s);
        }
        auto rg = rec::Load(gocpp::recv(pd->rg));
        if(rg != pdNil && rg != pdReady)
        {
            go_throw("runtime: blocked read on free polldesc"_s);
        }
        pd->fd = fd;
    ...
    void poll_runtime_pollClose(pollDesc* pd)
    {
        if(! pd->closing)
        {
            go_throw("runtime: close polldesc w/o unblock"_s);
        }
        auto wg = rec::Load(gocpp::recv(pd->wg));
        if(wg != pdNil && wg != pdReady)
        {
            go_throw("runtime: blocked write on closing polldesc"_s);
        }
        auto rg = rec::Load(gocpp::recv(pd->rg));
        if(rg != pdNil && rg != pdReady)
        {
            go_throw("runtime: blocked read on closing polldesc"_s);
        }
        netpollclose(pd->fd);
        rec::free(gocpp::recv(pollcache), pd);
    }

    void rec::free(pollCache* c, pollDesc* pd)
    {
    ...
    void poll_runtime_pollUnblock(pollDesc* pd)
    {
        ...
        if(pd->closing)
        {
            go_throw("runtime: unblock on closing polldesc"_s);
        }
        pd->closing = true;
        pd->rseq++;
        pd->wseq++;
    ...
    bool netpollblock(pollDesc* pd, int32_t mode, bool waitio)
    {
        ...
        if(old > pdWait)
        {
            go_throw("runtime: corrupted polldesc"_s);
        }
        return old == pdReady;
    }
