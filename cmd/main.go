...
func (cv *cppConverter) generateSortedHeader(headerElts []*place, getter func(place) []string, dm depMode, outFile outFile, inNamespace bool, keepTag tagType) bool {
	...
	if len(headerElts) != 0 {
		...
		topoSort(cv, headerElts, getter, outFile.name)
...
func (cv *cppConverter) startPerfScope(tag string) *perfScope {
	return createPerfScope(cv.getLogPerfFile(), tag, cv.genDepth)
}
...
func (cv *cppConverter) logReferencedTypesFrom(usedTypes map[types.Object]tagType, name string) {
	...
	for usedType, tag := range usedTypes {
		...
		var file = cv.pcShared.fileSet.Position(usedType.Pos())
...
func (cv *cppConverter) convertTypeExpr(node ast.Expr, ctx ctContext) cppType {
	...
	case *ast.IndexListExpr:
		...
		for _, index := range n.Indices {
			cppIndices = append(cppIndices, cv.convertTypeExpr(index, ctx))
		}
		cppType := ExprPrintf("%s<%s>", cv.convertTypeExpr(n.X, ctx), JoinExpr(cppIndices, ", ")).toCppType()
		return cppType
...
func (cv *cppConverter) addPkgDependencies(inputPath string) []*ast.File {
	...
	if len(pkgs) == 0 {
		cv.Panicf("Can't load packages from query %q", query)
	}
