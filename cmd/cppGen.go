...
func printCppIntro(cv *cppConverter, pkgInfos []*pkgInfo, receiversElts set[string], namespaceElts set[string]) {
	out := cv.cpp.out
	...
	fmt.Fprintf(out, "namespace %s::%v\n", goNs, cv.fullNamespace)
	fmt.Fprintf(out, "{\n")
	for _, elt := range toSortedList(namespaceElts) {
		fmt.Fprintf(out, "    %s;\n", elt)
	}
