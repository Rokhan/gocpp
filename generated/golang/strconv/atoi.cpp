...
namespace golang::strconv
{
    ...
    std::tuple<int64_t, std::string> ParseInt(std::string s, int base, int bitSize)
    {
        ...
        if(err != nullptr && gocpp::getValue<strconv.NumError*>(err)->Err != ErrRange)
        {
            int64_t i;
            std::string err;
            gocpp::getValue<strconv.NumError*>(err)->Func = fnParseInt;
            gocpp::getValue<strconv.NumError*>(err)->Num = cloneString(s0);
            return {0, err};
        }
        if(bitSize == 0)
        {
    ...
    std::tuple<int, std::string> Atoi(std::string s)
    {
        ...
        if(intSize == 32 && (0 < sLen && sLen < 10) || intSize == 64 && (0 < sLen && sLen < 19))
        {
            ...
            auto n = 0;
            for(auto [_, ch] : gocpp::Tag<gocpp::slice<unsigned char>>()(s))
            {
                ch -= '0';
                if(ch > 9)
                {
        ...
        if(auto [nerr, ok] = gocpp::getValue<strconv.NumError*>(err); ok)
        {
            nerr->Func = fnAtoi;
        }
        return {int(i64), err};
