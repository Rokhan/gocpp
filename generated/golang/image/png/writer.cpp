...
namespace golang::png
{
    ...
    bool opaque(image::Image m)
    {
        ...
        for(auto y = b.Min.Y; y < b.Max.Y; y++)
        {
            for(auto x = b.Min.X; x < b.Max.X; x++)
            {
                ...
                uint32_t _;
                uint32_t a;
                std::tie(_, _, _, a) = RGBA(gocpp::recv(At(gocpp::recv(m), x, y)));
                if(a != 0xffff)
                {
                    return false;
                }
    ...
    void writeChunk(encoder* e, gocpp::slice<unsigned char> b, std::string name)
    {
        ...
        std::tie(_, e->err) = Write(gocpp::recv(e->w), b);
        if(e->err != nullptr)
        {
            return;
        }
    ...
    std::string writeImage(encoder* e, io::Writer w, image::Image m, int cb, int level)
    {
        ...
        for(auto y = b.Min.Y; y < b.Max.Y; y++)
        {
                ...
                switch(conditionId)
                {
                    ...
                    case 1:
                        ...
                        else
                        {
                            for(auto x = b.Min.X; x < b.Max.X; x++)
                            {
                                auto [r, g, b, _] = RGBA(gocpp::recv(At(gocpp::recv(m), x, y)));
                                cr0[i + 0] = uint8_t(r >> 8);
                                cr0[i + 1] = uint8_t(g >> 8);
                                cr0[i + 2] = uint8_t(b >> 8);
                                i += 3;
            ...
            if(auto [_, err] = Write(gocpp::recv(e->zw), cr[f]); err != nullptr)
            {
                return err;
            }
