...
namespace golang::rand
{
    ...
    Rand* New(Source src)
    {
        auto [s64, _] = gocpp::getValue<Source64>(src);
    ...
    std::tuple<int, std::string> Read(struct Rand* r, gocpp::slice<unsigned char> p)
    {
        ...
        {
            const auto& gocpp_id_0 = gocpp::type_info(r->src);
            int conditionId = -1;
            if(gocpp_id_0 == typeid(lockedSource*)) { conditionId = 0; }
            else if(gocpp_id_0 == typeid(runtimeSource*)) { conditionId = 1; }
            switch(conditionId)
            {
                int n;
                std::string err;
    ...
    std::tuple<int, std::string> read(gocpp::slice<unsigned char> p, Source src, int64_t* readVal, int8_t* readPos)
    {
        ...
        auto pos = *readPos;
        auto val = *readVal;
        auto [rng, _] = gocpp::getValue<rngSource*>(src);
        for(n = 0; n < len(p); n++)
        {
            int n;
            std::string err;
    ...
    void Seed(int64_t seed)
    {
        ...
        if(orig != nullptr)
        {
            if(auto [_, ok] = gocpp::getValue<lockedSource*>(orig->src); ok)
            {
